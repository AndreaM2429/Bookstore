...
"Bookstore" is an app that allows you: Display a list of books, Add a new book, Remove a selected book.
...
```sh
  cd my-folder-name
  $ git clone https://github.com/AndreaM2429/Bookstore
```
...
```
**Note:** About the live server server you need to run the following command inside the project `Bookstore`.
```
npm start
```
...
Now you are able to modify the files in the `Bookstore/src`, but be careful just modify them, the rest are for the funcionality of the app so **NOT MODIFY THEM**.
